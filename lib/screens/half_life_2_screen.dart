...
class HalfLife2Screen extends StatefulWidget {
  @override
  _HalfLife2ScreenState createState() => _HalfLife2ScreenState();
}

class _HalfLife2ScreenState extends State<HalfLife2Screen> {
  ...
  Future<void> fetchData() async {
    ...
    if (jogoResponse.statusCode == 200) {
      ...
      // Se a resposta for uma lista, pegamos o primeiro item com jogoId == 2
      if (jogoData is List && jogoData.isNotEmpty) {
        final jogo = jogoData.firstWhere(
          (jogoItem) => jogoItem['jogoId'] == 2,
          orElse: () => null,
        );

        setState(() {
          this.jogo = jogo; // Pega o jogo com jogoId igual a 2
        });
      } else {
        setState(() {
          jogo = null; // Se não for uma lista válida, definimos jogo como null
        });
      }
    }
    ...
    if (dlcResponse.statusCode == 200) {
      ...
      final filteredDlcs = dlcData.where((dlc) => dlc['jogoId'] == 2).toList();

      setState(() {
        dlcs = filteredDlcs; // Filtra DLCs com jogoId igual a 2
      });
    }
    ...
    if (personagensResponse.statusCode == 200) {
      ...
      // Filtra personagens com jogoId igual a 1 ou 2
      final filteredPersonagens =
          personagensData
              .where(
                (personagem) =>
                    personagem['jogoId'] == 1 || personagem['jogoId'] == 2,
              )
              .toList();

      setState(() {
        personagens =
            filteredPersonagens; // Filtra personagens com jogoId 1 ou 2
      });
    }
    ...
    if (eventosResponse.statusCode == 200) {
      ...
      final filteredEventos =
          eventosData.where((evento) => evento['jogoId'] == 2).toList();

      setState(() {
        eventos = filteredEventos; // Filtra eventos com jogoId igual a 2
      });
    }
    ...
    if (mapasResponse.statusCode == 200) {
      ...
      final filteredMapas =
          mapasData.where((mapa) => mapa['jogoId'] == 2).toList();

      setState(() {
        mapas = filteredMapas; // Filtra mapas com jogoId igual a 2
      });
    }
  }
